...
\begin{frame}{Experimental setup}
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    Input handling  & Scratch models at 32 px; transfer CNNs at 96--160 px; ViT at 224 px \\
...
\begin{frame}{From-scratch controls on 20 coarse classes}
  ...
  \begin{tabular}{@{}lcccc@{}}
    ...
    \midrule
    Baseline CNN & image $(32,32,3)$ & 0.268 & 0.247 & 0.646 \\
...
\begin{frame}{Coarse: transfer learning lifts the floor}
  ...
  \end{columns}
  \vfill
  \source{ResNet50V2 / DenseNet121 / EfficientNetB0 / MobileNetV3-Small, coarse multiclass.}
\end{frame}
    ...
    \column{0.49\textwidth}
      \begin{itemize}
        \item Frozen head already reaches \textbf{0.683}; unfreezing block 6 adds
              \textbf{+0.083}, full fine-tune a further \textbf{+0.029}.
      \end{itemize}
    \column{0.49\textwidth}
      \begin{itemize}
        \item \textbf{+0.112 Macro F1} end-to-end --- the largest frozen$\to$tuned lift of any
...
\begin{frame}{When does unfreezing pay off?}
  \centering
  % fmt: height cap keeps image + bullet columns clear of the footer
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \column{0.49\textwidth}
      \begin{itemize}
        \item Largest ConvNet gain: \textbf{EfficientNetB0} ($0.683\rightarrow0.796$).
              ResNet and DenseNet show smaller gains in the exported runs.
      \end{itemize}
  \end{columns}
  \vfill
  \source{Per-architecture frozen $\rightarrow$ best fine-tuned Macro F1 (fine task).}
\end{frame}
...
\begin{frame}{Conclusions: one table, every architecture}
  ...
  \begin{tabular}{@{}lcccc@{}}
    ...
    \arch{EfficientNetB0}  & ---  & 0.739         & 0.796         & transfer  \\
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.5\textwidth}
      ...
      \begin{itemize}
        ...
        \item Transfer CNNs resize to 96--160\,px depending on the backbone.
...
\begin{frame}{Backup: hyperparameters of the headline runs}
  ...
  \begin{tabular}{@{}lccccc@{}}
    ...
    MobileNetV3-S (coarse)     & 96  & 30 & 1e-4 & 32 & frozen $\to$ unfreeze \\
    EfficientNetB0 (fine)      & 128 & 30--40 & 1e-4 / 1e-5 & 32 & frozen $\to$ block6 $\to$ full \\
    EfficientNetB3 (fine)      & 160 & 40 & 1e-5 & 16 & full fine-tune \\
    ResNet50V2 (coarse/fine)   & 160 & 15--20 & 1e-4 & 32 & frozen $\to$ fine-tune \\
    DenseNet121 (coarse/fine)  & 160 & 10--20 & 2e-6 & 32 & frozen $\to$ fine-tune \\
